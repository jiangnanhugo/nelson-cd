...
def iterate_batch(data, batch_size, unk_replace=0., shuffle=False):
    ...
    stack_keys = ['STACK_HEAD', 'CHILD', 'SIBLING', 'STACK_TYPE', 'SKIP_CONNECT', 'MASK_DEC']
    exclude_keys = set(['SINGLE', 'WORD', 'LENGTH', 'CONSTRAINT_POS_MASK', 'CON_RELATION_MASK'] + stack_keys)
    ...
    for start_idx in range(0, data_size, batch_size):
        ...
        batch = {'WORD': words[excerpt, :batch_length],
                 'LENGTH': lengths,
                 "CONSTRAINT_POS_MASK": data['CONSTRAINT_POS_MASK'][excerpt, :batch_length, :batch_length],
                 'CON_RELATION_MASK': data['CON_RELATION_MASK'][excerpt, :batch_length, :]}
...
def iterate_bucketed_batch(data, batch_size, unk_replace=0., shuffle=False):
    ...
    for bucket_id in bucket_indices:
        ...
        for start_idx in range(0, bucket_size, batch_size):
            ...
            batch.update({key: field[excerpt, :batch_length] for key, field in data.items() if key not in exclude_keys})
            batch.update(
                {key: field[excerpt, :2 * batch_length - 1] for key, field in data.items() if key in stack_keys})
            yield batch
