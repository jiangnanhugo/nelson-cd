...
from neuronlp2.io.utils import get_logger
    ...
    def expand_vocab():
        ...
        for data_path in data_paths:
            ...
            with open(data_path, 'r') as file:
                for line in file:
                    line = line.strip()
                    if len(line) == 0 or line.strip() == '' or line.strip().startswith('#'):
                        continue
                    tokens = line.split('\t')
...
def create_pos_consraint(pos_tag_constraint_filepath, upos_relation_constraint_filepath):
    cont = json.load(open(pos_tag_constraint_filepath, 'r'))
    rel_cont = json.load(open(upos_relation_constraint_filepath, 'r'))

    return cont, rel_cont


def read_data(source_path: str, word_alphabet: Alphabet, char_alphabet: Alphabet, pos_alphabet: Alphabet, type_alphabet: Alphabet,
              max_size=None, normalize_digits=True, symbolic_root=False, symbolic_end=False, constraint_pos_dict=None,
    ...
    inst = reader.getNext(normalize_digits=normalize_digits, symbolic_root=symbolic_root, symbolic_end=symbolic_end,
                          constraint_pos_dict=constraint_pos_dict, constraint_relation_dict=constraint_relation_dict)
    while inst is not None and (not max_size or counter < max_size):
        ...
        data.append([sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids,
                     inst.constraint_pos_mask, inst.con_relation_mask])
    ...
    cos_pos_msks = np.zeros([data_size, max_length, max_length], dtype=np.float32)
    con_relation_msks = np.zeros([data_size,  max_length,type_alphabet.size()], dtype=np.float32)
    single = np.zeros([data_size, max_length], dtype=np.int64)
    lengths = np.empty(data_size, dtype=np.int64)

    for i, inst in enumerate(data):
        wids, cid_seqs, pids, hids, tids, cos_pos_msk, con_relation_msk = inst
        inst_size = len(wids)
        lengths[i] = inst_size
        # word ids
        ...
        cos_pos_msks[i, :inst_size, :inst_size] = cos_pos_msk
        con_relation_msks[i, :inst_size, :] = con_relation_msk
    ...
    constraint_pos_masks = torch.from_numpy(cos_pos_msks)
    con_relation_msks = torch.from_numpy(con_relation_msks)

    data_tensor = {'WORD': words, 'CHAR': chars, 'POS': pos, 'HEAD': heads, 'TYPE': types,
                   'MASK': masks, 'SINGLE': single, 'LENGTH': lengths, 'CONSTRAINT_POS_MASK': constraint_pos_masks,
                   'CON_RELATION_MASK': con_relation_msks}
...
def read_bucketed_data(source_path: str, word_alphabet: Alphabet, char_alphabet: Alphabet, pos_alphabet: Alphabet, type_alphabet: Alphabet,
                       ...
                       constraint_pos_dict=None, constraint_relation_dict=None):
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        if counter % 10000 == 0:
            pass
            # print("reading data: {}".format(counter))
    ...
    for bucket_id in range(len(_buckets)):
        ...
        con_upos_msks = np.zeros([bucket_size, bucket_length, bucket_length], dtype=np.float32)
        con_relation_msks = np.zeros([bucket_size, bucket_length, type_alphabet.size()], dtype=np.float32)
        ...
        for i, inst in enumerate(data[bucket_id]):
            ...
            # masks
            masks[i, :inst_size] = 1.0
            con_upos_msks[i, :inst_size, :inst_size] = cos_pos_msk
            con_relation_msks[i, :inst_size, :] = con_relation_msk
            for j, wid in enumerate(wids):
                if word_alphabet.is_singleton(wid):
                    single[i, j] = 1
        ...
        single = torch.from_numpy(single)
        lengths = torch.from_numpy(lengths)
        constraint_pos_masks = torch.from_numpy(con_upos_msks)
        con_relation_msks = torch.from_numpy(con_relation_msks)
        # print("con_relation_msks:", con_relation_msks.shape)

        data_tensor = {'WORD': words, 'CHAR': chars, 'POS': pos, 'HEAD': heads, 'TYPE': types,
                       'MASK': masks, 'SINGLE': single, 'LENGTH': lengths, 'CONSTRAINT_POS_MASK': constraint_pos_masks,
                       'CON_RELATION_MASK': con_relation_msks}
        data_tensors.append(data_tensor)
    print("reading data_tensors: {}".format(counter))
