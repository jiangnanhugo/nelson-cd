...
class ConstraintDeepBiAffine(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, rnn_mode, hidden_size, num_layers,
        ...
        super(ConstraintDeepBiAffine, self).__init__()
    ...
    def loss(self, input_word, input_char, input_pos, heads, types, mask=None, constraint_pos_msk=None, con_relation_masks=None):
        ...
        if constraint_pos_msk is not None:
            pos_constraint_mask=constraint_pos_msk.eq(0)
            out_arc = out_arc.masked_fill(pos_constraint_mask, float('-inf'))

        # loss_arc shape [batch, length_c]
        logged = F.log_softmax(out_arc, dim=1)
        loss_arc = self.nll(logged, heads)
        loss_arc = torch.nan_to_num(loss_arc, nan=0.0)



        if con_relation_masks is not None:
            con_relation_masks = con_relation_masks.eq(0)
            # out_type = out_type.masked_fill(con_relation_masks, float('-inf'))

        out_type = out_type.transpose(1, 2)
        loss_type = self.nll(F.log_softmax(out_type, dim=1), types)
        loss_type = torch.nan_to_num(loss_type, nan=0.0)

        # old_loss_type = self.criterion(out_type, types)
    ...
    def _decode_types(self, out_type, heads, leading_symbolic, con_relation_masks=None):
        # out_type shape [batch, length, type_space]
        type_h, type_c = out_type
        # get vector for heads [batch, length, type_space],
        type_h = type_h.gather(dim=1, index=heads.unsqueeze(2).expand(type_h.size()))
    ...
    def decode_local(self, input_word, input_char, input_pos, mask=None, leading_symbolic=0, constraint_pos_msk=None, con_relation_masks=None):
        ...
        if constraint_pos_msk is not None:
            minus_constraint_mask = constraint_pos_msk.eq(0)
            out_arc = out_arc.masked_fill(minus_constraint_mask, float('-inf'))
        # compute naive predictions.
        # predition shape = [batch, length_c]

        sec, heads = out_arc.max(dim=1)
        types = self._decode_types(out_type, heads, leading_symbolic, con_relation_masks)
        return heads.cpu().numpy(), types.cpu().numpy()

    def decode(self, input_word, input_char, input_pos, mask=None, leading_symbolic=0, constraint_pos_msk=None, constarint_relation_msk=None):
        """ Args:
            input_word: Tensor. the word input tensor with shape = [batch, length]
            input_char: Tensor. the character input tensor with shape = [batch, length, char_length]
            input_pos: Tensor. the pos input tensor with shape = [batch, length]
            mask: Tensor or None. the mask tensor with shape = [batch, length]
            length: Tensor or None.  the length tensor with shape = [batch]
            hx: Tensor or None. the initial states of RNN
            leading_symbolic: int. number of symbolic labels leading in type alphabets (set it to 0 if you are not sure)
            Returns: (Tensor, Tensor). predicted heads and types.
        """
        # out_arc shape [batch, length_h, length_c]
        out_arc, out_type = self(input_word, input_char, input_pos, mask=mask)
        ...
        if constraint_pos_msk is not None:
            minus_constraint_mask = constraint_pos_msk.eq(0)#.permute(0, 2, 1)
            out_arc = out_arc.masked_fill(minus_constraint_mask, float('-inf'))

        # loss_arc shape [batch, length_h, length_c]
        loss_arc = F.log_softmax(out_arc, dim=1)
        loss_arc = torch.nan_to_num(loss_arc)
        print("decode mst:\n", loss_arc[0])
