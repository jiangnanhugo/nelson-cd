...
from neuronlp2.nn import NCETreeCRF, VarGRU, VarRNN, VarLSTM, VarFastLSTM
from neuronlp2.nn import BiAffine, BiLinear, CharCNN
from neuronlp2.tasks import parser
from neuronlp2.models import DeepBiAffine


class SampledMST(DeepBiAffine):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, rnn_mode, hidden_size, num_layers,
        ...
        super(SampledMST, self).__init__(word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, rnn_mode,
        ...
        self.biaffine = None
        self.treecrf = NCETreeCRF(arc_space)
    ...
    def sampled_loss(self, input_word, input_char, input_pos, heads, types, mask=None):
        # output from rnn [batch, length, dim]
        arc, out_type = self._get_rnn_output(input_word, input_char, input_pos, mask=mask)
        # [batch]
        loss_arc = self.treecrf.sampled_loss(arc[0], arc[1], heads, mask=mask)
        # print("loss_arc:", loss_arc)
        # out_type shape [batch, length, type_space]
        type_h, type_c = out_type
    ...
    @overrides
    def decode(self, input_word, input_char, input_pos, mask=None, leading_symbolic=0, constraint_pos_msk=None):
        """
        Args:
            input_word: Tensor
        ...
        heads, _ = parser.decode_MST(energy.cpu().numpy(), length.cpu().numpy(),
