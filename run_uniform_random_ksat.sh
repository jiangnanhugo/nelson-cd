...
do
	...
	if [[ "$algo" == "nelson" ]]; then
		echo "nelson $i"
		timeout 60 python3 nelson/sat_uniform_gen.py --samples $NUM --input $input_file --algo nelson --K 5 > $output_file.nelson.log
	fi

	if [[ "$algo" == "lll" ]]; then
		echo "$algo $i"
		timeout 60 python3 nelson/sat_uniform_gen.py --samples $NUM --input $input_file --algo lll --K 5 > $output_file.$algo.log
	fi

	if [[ "$algo" == "prs" ]]; then
		echo "$algo $i"
		timeout 60 python3 nelson/sat_uniform_gen.py --samples $NUM --input $input_file --algo prs --K 5 > $output_file.$algo.log
	fi

	if [[ "$algo" == "numpy" ]]; then
		echo "$algo $i"
		timeout 60 python3 nelson/sat_uniform_gen.py --samples $NUM --input $input_file --algo numpy --K 5 > $output_file.$algo.log
	fi
