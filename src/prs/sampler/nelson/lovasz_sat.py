...
def constructive_lovasz_local_lemma_sampler(instance, prob=None, max_tryouts=10000, **kwargs):
    """
...
def partial_rejection_sampling_sampler(instance, prob=None, max_tryouts=10000, **kwargs):
    """
...
def pytorch_neural_lovasz_sampler(instance, clause2var, weight, bias, device, prob=0.5, max_tryouts=10000, **kwargs):
    """
...
def pytorch_batch_neural_lovasz_sampler(instance, clause2var, weight, bias, device, prob=0.5, max_tryouts=10000, **kwargs):
    """
    ...
    st = time.time()
    # start with a random assignment for all variables.
    init_rand = torch.rand((batch_size, instance.cnf.nv)).to(device)
    ...
    for it in range(max_tryouts):
        ...
        violated_RV = (violated_RV > 0).int()
        resampled_rand = torch.rand((batch_size, instance.cnf.nv)).to(device)
...
def numpy_neural_lovasz_sampler(instance, clause2var, weight, bias, prob=0.5, max_tryouts=10000, **kwargs):
    """
...
def numpy_batch_neural_lovasz_sampler(instance, clause2var, weight, bias, prob=0.5, max_tryouts=10000, **kwargs):
    """
    Lovasz SAT Sampler using Numpy implementation with extreme condition for the input SAT-CNF.
    We start with a random assignment for all variables. Then, we find the all the violated clauses,
    and resample the variables in all those clause (assigning a new values to each of them IID).
    ...
    batch_size = kwargs['batch_size']
    # start with a random assignment for all variables.
    init_rand = np.random.rand(batch_size, instance.cnf.nv)
    ...
    for it in range(max_tryouts):
        ...
        if np.sum(violated_RV) == 0:
            return assignment, it + 1, time.time() - st
        uniform_rand = np.random.rand(batch_size, instance.cnf.nv)
