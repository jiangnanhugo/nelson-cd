...
def compute_distance(samples, model, num_samples, nabla_logZ):
    ...
    summed_phi.backward()
    with torch.no_grad():
        apx_nabla_logZ = model.theta.grad / num_samples
        print("{:.6f}: {}".format(torch.linalg.norm(apx_nabla_logZ - nabla_logZ), apx_nabla_logZ.cpu().numpy()))
...
def compute_nabla_log_ZC(instance, model, num_samples, input_file, **kwargs):
    ...
    nabla_logZ /= ZC
    #
    print('True:', nabla_logZ.cpu().numpy())
    samples = draw_from_waps(num_samples, input_file, instance, prob)
    print('waps:', end=" ")
    compute_distance(samples, model, num_samples, nabla_logZ)
    #
    samples = draw_from_quicksampler(num_samples, input_file)
    print('quicksampler:', end=" ")
    compute_distance(samples, model, num_samples, nabla_logZ)
    ...
    samples = draw_from_prs_series('nelson_batch', instance, clause2var, weight, bias, prob, num_samples)
    samples = [x.reshape(1, -1) for x in samples]
    print('nelson_batch:', end=" ")
    compute_distance(samples, model, num_samples, nabla_logZ)
    #
    samples = draw_from_prs_series('nelson', instance, clause2var, weight, bias, prob, num_samples)
    samples = [x.reshape(1, -1) for x in samples]
    ...
    samples = draw_from_prs_series('numpy', instance, clause2var, weight, bias, prob.cpu().numpy(), num_samples)
    samples = [x.reshape(1, -1) for x in samples]
    # print(samples[0].shape)
    print('numpy:', end=" ")
    compute_distance(samples, model, num_samples, nabla_logZ)
    #
    #
    # samples = draw_from_weightgen(40, input_file, instance, prob)
    # new_inputs = torch.concat(samples, dim=0).to(device)
    # phi = model(new_inputs)
    # summed_phi = torch.sum(phi)
    # model.zero_grad()
    # summed_phi.backward()
    # with torch.no_grad():
    #     apx_nabla_logZ = model.theta.grad / 40
    #     print('weightgen:', apx_nabla_logZ)
    #     print("weightgen dist: {:.6f}".format(torch.linalg.norm(apx_nabla_logZ - nabla_logZ)))
    exit()
